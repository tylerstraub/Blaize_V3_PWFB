...
public class Blaize_V3_PWFB extends PApplet {
...
int frameSizeX = 1440;                    // width of drawin zone only (without (!) command window, 'cmdWindowWidth' pixels wide)
int frameSizeY = 900;                   // height of window
...
public void draw() { 
  ...
  if (!O[1].buttonState) {      // if no Blackout
    presetBrightness += 0.2f*(presetBrightnessDestination-presetBrightness);
  }

  presetSize += 0.14f*(presetSizeDestination-presetSize);
  ...
  if (drawTranslateMode  &&  presetPos.size() >= 3) {
    translateCounter += 1.0f;
    if (translateCounter >= presetPos.size()) {  
      translateCounter = 0;
    }
    translate(map(presetPos.get(PApplet.parseInt(translateCounter)).x, LMZxpos+frameSizeX, LMZxpos+frameSizeX+LMZxsize, -frameSizeX, frameSizeX), map(presetPos.get(PApplet.parseInt(translateCounter)).y, LMZypos, LMZypos+LMZysize, -height, height));
  }
  ...
  if (bpmSTLmode) {
    if (millis()-bpmSTLtime >= bpmConstant/bpm) {
      ...
      int t = PApplet.parseInt(random(10, 95));     // No easing, instant size change
      X[1].value = t;
      presetSize = t;
        ...
        do {
          ...
          if (presetNumber == 8) {        // Mini-Dots-Discoball neu initialisieren
            for (int x=0; x<1000; x++) {
              ranX[x] = PApplet.parseInt(random(-0.71f*frameSizeX, 0.71f*frameSizeX));
              ranY[x] = PApplet.parseInt(random(-0.71f*frameSizeX, 0.71f*frameSizeX));
            }
          }
  ...
  if (millis()-strobeTime >= 200-1.8f*presetStrobing) {  
    onoff = !onoff; 
    strobeTime = millis();
  }
  if (presetStrobing == 0) {  
  ...
  if (onoff) {
    ...
    case 0:  
      ...
      for (float i=0; i<18; i++) {
        ...
        ellipse(frameSizeX/2+((0.9f*presetSize+10)*height/240)*sin(i*v), height/2+((0.9f*presetSize+10)*height/240)*cos(i*v), 0.8f*(0.9f*presetSize+10), 0.8f*(0.9f*presetSize+10));  
        v += PApplet.parseFloat(presetSpeed)/360000;
      }
      break;
    ...
    case 1:  
      ...
      rotate(v); 
      v += PApplet.parseFloat(presetSpeed)/2000;
      for (float i=0; i<18; i++) {
        if (multiColor  && i%2 == 0) {  
          fill(presetColor, presetBrightness*255/100);
        } else if (multiColor) {  
        ...
        ellipse(((0.9f*presetSize+10)*height/240)*sin(i*PI/9), ((0.9f*presetSize+10)*height/240)*cos(i*PI/9), 0.8f*(0.9f*presetSize+10), 0.8f*(0.9f*presetSize+10));
      }
      break;
      /* 3 Kreis-Ringe */
    case 2:  
      ...
      translate(frameSizeX/2, height/2);
      rotate(v); 
      v += PApplet.parseFloat(presetSpeed)/3000;
      for (float i=0; i<18; i++) {
        if (multiColor  && i%2 == 0) {  
          fill(presetColor, presetBrightness*255/100);
        } else if (multiColor) {  
        ...
        ellipse(((0.9f*presetSize+10)*height/240)*sin(i*PI/9), ((0.9f*presetSize+10)*height/240)*cos(i*PI/9), 0.4f*(0.9f*presetSize+10), 0.4f*(0.9f*presetSize+10));
      }
      for (float i=0; i<18; i++) {
        if (multiColor  && i%2 == 0) {  
          fill(presetColor, presetBrightness*255/100);
        ...
        ellipse(((0.9f*presetSize+10)*height/800)*sin(i*PI/9), ((0.9f*presetSize+10)*height/800)*cos(i*PI/9), 0.1f*(0.9f*presetSize+10), 0.1f*(0.9f*presetSize+10));
      }
      rotate(-2*v);
      for (float i=0; i<18; i++) {
        if (multiColor  && i%2 == 0) {  
        ...
        ellipse(((0.9f*presetSize+10)*height/400)*sin(i*PI/9), ((0.9f*presetSize+10)*height/400)*cos(i*PI/9), 0.2f*(0.9f*presetSize+10), 0.2f*(0.9f*presetSize+10));
      }                 
      break;
      /* Random Linien */
    case 3:  
      strokeWeight(1.4f*presetSize+10);
      if (multiColor) {
      ...
      if (millis()-time >= 1000-10*presetSpeed  &&  presetSpeed != 0) {
        time = millis();
        a = PApplet.parseInt(random(50, frameSizeX-50));
        b = PApplet.parseInt(random(50, height-50));
        c = PApplet.parseInt(random(50, frameSizeX-50));
        d = PApplet.parseInt(random(50, height-50));
      }
      break;
    ...
    case 4:  
      ...
      if (millis()-time >= 1000-10*presetSpeed  &&  presetSpeed != 0) {
        time = millis();
        a = PApplet.parseInt(random(150, frameSizeX-150));
        b = PApplet.parseInt(random(150, height-150));
      }
      break;
      /* Sinus */
    case 5:  
      ...
      for (float f=0; f<frameSizeX/16; f++) {
        ...
        ellipse(16*f, height/2+ 4.5f*presetSize*sin(v - f/15 /*NumOfPeriods*/), 70, 70);  
        v += PApplet.parseFloat(presetSpeed)/90000;
      }
      break;
      /* Sinus-Blöcke */
    case 6:  
      ...
      for (float f=0; f<frameSizeX/90; f++) {
        ...
        rect(100*f, 0.45f*height+100*sin(v - f/1)+presetSize*height*0.005f, 60, height/10);
        rect(100*f, 0.45f*height-100*sin(v - f/1)-presetSize*height*0.005f, 60, height/10);  
        v += PApplet.parseFloat(presetSpeed)/15000;
      }
      break;
      /* Dreh-Kreuz */
    case 7:  
      strokeWeight(1.5f*presetSize);
      translate(frameSizeX/2, height/2);
      rotate(v); 
      v += PApplet.parseFloat(presetSpeed)/1000;
      if (multiColor) {
    ...
    case 8:  
      ...
      translate(frameSizeX/2, height/2);
      rotate(v);  
      v += PApplet.parseFloat(presetSpeed)/5000;
      for (int x=0; x<1000; x++) {
        if (multiColor && x>500) {  
          fill(multiColorclr, presetBrightness*255/100);
        }
    ...
    case 9:  
      ...
      for (int x=0; x<(presetSize+10)/10; x++) {
        ...
        ellipse(ranX[x], ranY[x], 300-2.7f*presetSize, 300-2.7f*presetSize);
      }
      if (millis()-time >= 1000-9.5f*presetSpeed  &&  presetSpeed != 0) {
        time = millis();
        for (int x=0; x<(presetSize+10)/10; x++) {
          ranX[x] = PApplet.parseInt(random((300-2.7f*presetSize)/2, frameSizeX-(300-2.7f*presetSize)/2));
          ranY[x] = PApplet.parseInt(random((300-2.7f*presetSize)/2, height    -(300-2.7f*presetSize)/2));
        }
      }
      break;
      /* Random Ringe */
    case 10: 
      noFill();
      strokeWeight(60-0.5f*presetSize);
      for (int x=0; x<(presetSize+10)/10; x++) {
        if (multiColor && x>presetSize/20) {  
          stroke(multiColorclr, presetBrightness*255/100);
        }
    ...
    case 11: 
      strokeWeight(1.5f*presetSize);
      translate((frameSizeX/2.4f)*sin(v)+frameSizeX/2, 0); 
      v += PApplet.parseFloat(presetSpeed)/1000;
      if (multiColor) {
        line(0, 0, 0, height/3);
    ...
    case 12: 
      strokeWeight(1.5f*presetSize);
      translate(0, (height/2.4f)*sin(v)+height/2); 
      v += PApplet.parseFloat(presetSpeed)/1000;
      if (multiColor) {
    ...
    case 13: 
      strokeWeight(1.5f*presetSize);
      translate((frameSizeX/2.4f)*sin(v*1.2f)+frameSizeX/2, (height/2.4f)*sin(v*0.8f)+height/2); 
      v += PApplet.parseFloat(presetSpeed)/1000;
      if (multiColor) {
        for (float i=0; i<10; i++) {
          if (multiColor  && i%2 == 0) {  
    ...
    case 16: 
      ...
      m = m + PApplet.parseFloat(presetSpeed)/3;
      if (m >= frameSizeX-50) {  
        m = -50;
      }
      break;
    ...
    case 18: 
      ...
      m = m + PApplet.parseFloat(presetSpeed)/4;
      if (m >= frameSizeX-50) {  
        m = -50;
      }
      break;
    ...
    case 20: 
      ...
      v = v + PApplet.parseFloat(presetSpeed)/20;  
      if (v >= 100) {
        v = 0;
        a = PApplet.parseInt(random(0, 21));
        b = PApplet.parseInt(random(0, 21));
        c = PApplet.parseInt(random(350, height-50));
      }
      break;      
      /* Sinus mit Strahlen */
    case 21: 
      ...
      for (float f=0; f<frameSizeX/16; f++) {
        ...
        ellipse(16*f, height/2+4.5f*presetSize*sin(v - f/15 /*NumOfPeriods*/), 80, 80); 
        v += PApplet.parseFloat(presetSpeed)/90000;
      }
      break;
      /* Rotierende Lines */
    case 22: 
      strokeWeight(50+0.5f*presetSize);
      translate(frameSizeX/2, height/2);
      rotate(v); 
      v += PApplet.parseFloat(presetSpeed)/3000;
      for (int r=-8; r<=8; r++) {
    ...
    case 26: 
      ...
      for (int r=0; r<=3; r++) {
        ...
        for (float i=0; i<18; i++) {
          ...
          ellipse(((presetSize*0.6f+3)*height/120)*sin(i*PI/9), ((presetSize*0.6f+3)*height/120)*cos(i*PI/9), 0.5f*presetSize+6, 0.5f*presetSize+6);
        }
        popMatrix();
      }
      v += PApplet.parseFloat(presetSpeed)/4000;
      break;
      /* 4 crazy Kreis-Ringe */
    case 27: 
      noStroke();
      for (int xx=0; xx<=1; xx++) {
        for (int yy=0; yy<=1; yy++) {
          rotate(v); 
          v += PApplet.parseFloat(presetSpeed)/20000;
          for (float i=0; i<18; i++) {
            if (multiColor  && i%2 == 0) {  
              fill(presetColor, presetBrightness*255/100);
            } else if (multiColor) {  
            ...
            ellipse((2*xx+1)*(frameSizeX/4)+((presetSize+40)*height/240)*sin(i*PI/9), (2*yy+1)*(height/4)+((presetSize+40)*height/240)*cos(i*PI/9), 0.3f*(presetSize+40), 0.3f*(presetSize+40));
          }
        }
      }
      break;
    ...
    case 28: 
      ...
      v += PApplet.parseFloat(presetSpeed)/800;
      break;
      /* Tunnelrechteck */
    case 29: 
      noFill();
      ...
      m += PApplet.parseFloat(presetSpeed)/40;
      if (m >= 220) {  
        m = 0;
      }
      break;
    ...
    case 30: 
      ...
      for (int i=0; i<=9; i++) {
        for (int j=0; j<=9; j++) {
          if (multiColor) {
            if ((i+j)%2 == 0) {  
              fill(presetColor, constrain(-100+355*sin(v+31*PApplet.parseFloat(i+j)), 0, presetBrightness*255/100));
            } else {              
              fill(multiColorclr, constrain(-100+355*sin(v+31*PApplet.parseFloat(i+j)), 0, presetBrightness*255/100));
            }
          } else {
            fill(presetColor, constrain(-100+355*sin(v+31*PApplet.parseFloat(i+j)), 0, presetBrightness*255/100));
          }
      ...
      v += PApplet.parseFloat(presetSpeed)/1200;
      break;
    ...
    case 31: /*QuadPhaseLED.beginDraw();
      ...
      for (int b=0; b<=2; b++) {
        ...
        final float THRESH = 0.2f;           // Controls flash style of the 3 circles
        final float BLINKSPD = 3.0f;
        pushMatrix();
        switch(b) {
        case 0: 
          ...
          if (sin(BLINKSPD*v) > THRESH) {
            BRIGHT = map(sin(BLINKSPD*v), THRESH, 1.0f, 100, 0);
          }
          break;
        case 1: 
          ...
          if (sin(BLINKSPD*v+2*PI/3) > THRESH) {
            BRIGHT = map(sin(BLINKSPD*v+2*PI/3), THRESH, 1.0f, 100, 0);
          }
          break;
        case 2: 
          ...
          if (sin(BLINKSPD*v+4*PI/3) > THRESH) {
            BRIGHT = map(sin(BLINKSPD*v+4*PI/3), THRESH, 1.0f, 100, 0);
          }
          break;
        }
      ...
      v += map(PApplet.parseFloat(presetSpeed), 0, 100, -0.06f, 0.06f);
      rectMode(CORNER);
      break;
  ...
  if (millis()-time2 < 8000  &&  dontShowStartupScreenAnymore == false) {
    ...
    if (CC != null) {
      ...
      for (int k=0; k<vals1.length; k++) {
        ...
        if (vals2.length == 2) {
          _adress = PApplet.parseInt(vals2[0]);
          println("_adress: " + _adress);
          _data   = PApplet.parseInt(vals2[1]);
          println("_data: " + _data);
        } else {
          _adress = -1;
          _data   = -1;
...
public void mouseMoved() {              // Mouse moved while not clicked
  hideControlWindow = false;
  lastMouseMovedTime = millis();
}


public void mouseDragged() {            // Mouse moved while clicked
  hideControlWindow = false;
  lastMouseMovedTime = millis();
}
...
public void drawQuadLED(int _x, int _y, int _bright) {
...
class kreis {    // PingPong-Spotlight-Balls
  ...
  public void move() {
    int ballSize = PApplet.parseInt(map(presetSize+1, 0, 100, 300, 40));
...
class rectbutton {
  ...
  rectbutton(int _clr, int _xpos, int _ypos, int _xsize, int _ysize, int _index, String _modifier) {
    clr      = _clr;
    xpos     = _xpos;
    ypos     = _ypos;
    xsize    = _xsize;
...
class onoffbutton {
  ...
  onoffbutton(int _clr, int _xpos, int _ypos, int _xsize, int _ysize, int _index, String _modifier) {
    clr    = _clr;
    xpos   = _xpos;
    ypos   = _ypos;
    xsize  = _xsize;
...
class slider {
  ...
  slider(int _bgclr, int _clr, int _xpos, int _ypos, int _xsize, int _ysize, String _modifier) {
    bgclr  = _bgclr;
    clr    = _clr;
    xpos   = _xpos;
    ypos   = _ypos;
  ...
  public void display() {
    ...
    if (mouseX>xpos+frameSizeX && mouseX<(xpos+frameSizeX+xsize) && mouseY>ypos && mouseY<(ypos+ysize)) {
      ...
      textSize(ysize/3); 
      fill(130);
      text(PApplet.parseInt(constrain(map(mouseX, xpos+frameSizeX, xpos+frameSizeX+xsize, -5, 105), 0, 100)), xmiddle+frameSizeX, ymiddle-4);
      if (mousePressed  &&  mouseButton == LEFT) {
        flag = false;
        value = PApplet.parseInt(constrain(map(mouseX, xpos+frameSizeX, xpos+frameSizeX+xsize, -5, 105), 0, 100));
        doStuff();
      }
    } else {  
      fill(clr);
  ...
  public void settings() {  fullScreen(targetDisplay); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--hide-stop", "Blaize_V3_PWFB" };
    if (passedArgs != null) {
